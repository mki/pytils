...
pytils умеет выбирать правильный падеж в зависимости от числа
    ...
    >>> pytils.numeral.choose_plural(15, ("гвоздь", "гвоздя", "гвоздей"))
    u'гвоздей'
...
Часто нужен не просто вариант, а число вместе с текстом
    ...
    >>> pytils.numeral.get_plural(15, "гвоздь, гвоздя, гвоздей")
    u'15 гвоздей'

В get_plural можно еще передать вариант, когда число -- ноль. Т.е. чтобы
было не '0 гвоздей', а 'гвоздей нет':

    >>> pytils.numeral.get_plural(0, "гвоздь, гвоздя, гвоздей")
    u'0 гвоздей'
    >>> pytils.numeral.get_plural(0, "гвоздь, гвоздя, гвоздей", absence="гвоздей нет")
    u'гвоздей нет'
...
Также pytils реализует числа прописью
    ...
    >>> pytils.numeral.sum_string(32, pytils.numeral.MALE, ("гвоздь", "гвоздя", "гвоздей"))
...
В pytils можно получить русские даты без использования локалей.
    ...
    >>> pytils.dt.ru_strftime("сегодня - %d %B %Y, %A", inflected=True, date=datetime.date(2006, 9, 2))
    u'сегодня - 2 сентября 2006, суббота'

    >>> pytils.dt.ru_strftime("сделано %A, %d %B %Y", inflected=True, preposition=True)
...
При помощи pytils можно сделать транслитерацию:
    ...
    >>> print(pytils.translit.translify("Проверка связи"))
    'Proverka svyazi'
...
дефисы):
    ...
    >>> pytils.translit.slugify("тест и еще раз тест")
